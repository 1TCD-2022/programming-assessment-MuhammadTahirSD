...
print("Welcome to my football quiz, you will be asked 10 multiple choice questions about the footballing world. Please enter the number next to your desired answer or enter the full word answer (check your spelling), Good Luck!")
...
Answers = ["1", "3", "2", "2", "1", "3", "3", "3", "2", "1"]
Answers_words = ["manchester city", "real madrid", "manchester united", "barcelona", "cristiano ronaldo", "lionel messi", "five", "sir alex ferguson", "brazil", "spain"]

score = 0
for i in range(10): # Main quiz
    print(Questions[i])
    if i == 0: # Q1
    ...
    while validation:
        try:
            user_answer = str(input("Enter the answer: ")).lower().strip()
            try:
                user_answer_int = (int(user_answer))
                if user_answer_int in range(1,4):
                    pass
                else:
                    print("Please enter a number between 1-3 or the worded answer")
                    continue
            except:
                pass
            if user_answer == Answers[i] or user_answer == Answers_words[i]:
            ...
            print("Enter an actual answer please?")
